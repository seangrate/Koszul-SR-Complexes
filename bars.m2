...
new ProgressBar from Set := (typeofProgressBar, theSet) -> (
    ...
    theBar.TotalIterations = #theSequence;
    theBar
)
...
next ProgressBar := (theBar) -> (
    ...
    if theBar.cache.CurrentIndex > 0 then (
        theBar.cache.TotalTimeElapsed = cpuTime() - theBar.cache.StartTime;
    )
    else (
        theBar.cache.StartTime = cpuTime();
        theBar.cache.TotalTimeElapsed = theBar.cache.StartTime;
    );
